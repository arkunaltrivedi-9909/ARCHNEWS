// Smoke test for codes.html — boots the real page in a real browser against a
...
function serve(port) {
  return new Promise(resolve => {
    const srv = http.createServer((req, res) => {
      ...
      const file = path.join(ROOT, url === '/' ? 'codes.html' : url);
...
(async () => {
  ...
  await page.goto(`http://127.0.0.1:${PORT}/codes.html`, { waitUntil: 'networkidle' });
      ...
      rules: [
        {
          ...
          citation: { doc_id: 'fixture-sample', number: '2.1', page_label: 'p. 2',
        ...
        { // unverified but cited — must apply, with a visible warning
          ...
          when: { city: 'ahmedabad' },
          citation: { doc_id: 'fixture-sample', number: '3.2', page_label: 'p. 3' },
          verified: false
        }
      ]
    }, null, 2));
  ...
  // When the answer service is unreachable, the tool must degrade to showing
  // the retrieved clauses — never to answering from the model's own memory.
