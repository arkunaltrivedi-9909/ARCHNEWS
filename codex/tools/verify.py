...
def main():
    ...
    for doc_id in sorted(os.listdir(DOCS)):
        ...
        if man.get("type") == "amendment":
            n = man.get("amendment_items", 0)
            base = man.get("amends_doc")
            if not n:
                errors.append(f"{doc_id}: amendment parsed no edits — the locator pattern "
                              f"did not match this notification's format.")
            if not base:
                warnings.append(f"{doc_id}: not linked to a base document (--amends), so its "
                                f"{n} edit(s) cannot overlay the clauses they change.")
            elif base not in os.listdir(DOCS):
                warnings.append(f"{doc_id}: amends '{base}', which is NOT ingested. Its {n} "
                                f"edit(s) are listed but not applied to any clause — the base "
                                f"text will read as current when it is not.")
        elif not secs:
